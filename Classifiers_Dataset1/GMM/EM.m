function [new_coeff,Means_new,Covar_new] = EM(num_clusters,dataset)
    [clusters] = kmeansclustering(num_clusters,dataset);
    [N,dim] = size(dataset);
    ...
    for n=1:N
        ...
        for q = 1:num_clusters
            S = S + coeff(q)*probdens(X,mean(clusters(q)),dim,cov(clusters(q)));
        end
        for q = 1:num_clusters
            w(n,q)=coeff(q)*probdens(X,mean(clusters(q)),dim,cov(clusters(q)))/S;
        end
    end
...
function val = probdens(x,Mean,d,CoVarMatrix) 
   %val = (1/(((2*pi)^(d/2))*((det(CoVarMatrix))^(1/2))))*(exp(-(x-Mean)*(inv(CoVarMatrix))*(transpose(x-Mean)/2)));
