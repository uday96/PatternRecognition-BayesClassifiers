%Naive Bayes Classifier for Overlapping Data assuming same
%Covariance Matrix, C = sigma^2I
...
for i=1:4
    confusion_matrix(i,:) = assign_class(Datasets_test{i},Means,i);
...
for i=1:length(mesh_xy)
   ...
   for idx=1:4     
       classassigns(idx)=discriminant_func(X,Means{idx});
   end
...
function no_of_assignments = assign_class(class_test,Means,Actualclasslabel)
    global outputmatrix;
    global targetmatrix;
    global pointer;
    ...
    for i = 1:l_test
        ...
        disc_val = zeros(1,4);
        X = class_test(i,:);
        disc_val(1) = discriminant_func(X,Means{1});
        disc_val(2) = discriminant_func(X,Means{2});
        disc_val(3) = discriminant_func(X,Means{3});
        disc_val(4) = discriminant_func(X,Means{4});
...
function val = discriminant_func(x,Mean)
