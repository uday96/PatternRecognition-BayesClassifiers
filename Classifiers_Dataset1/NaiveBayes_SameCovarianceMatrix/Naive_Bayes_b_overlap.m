...
CoVarMatrix = get_cov([Datasets_train{1};Datasets_train{2};Datasets_train{3};Datasets_train{4}]);
...
for i=1:4
    confusion_matrix(i,:) = assign_class(Datasets_test{i},Means,CoVarMatrix,i);
...
for i=1:length(mesh_xy)
   ...
   for idx=1:4     
       classassigns(idx)=discriminant_func(X,Means{idx},CoVarMatrix);
   end
...
function no_of_assignments = assign_class(class_test,Means,CoVarMatrix,Actualclasslabel)
    global outputmatrix;
    global targetmatrix;
    global pointer;
    ...
    for i = 1:l_test
        ...
        disc_val = zeros(1,4);
        X = class_test(i,:);
        disc_val(1) = discriminant_func(X,Means{1},CoVarMatrix);
        disc_val(2) = discriminant_func(X,Means{2},CoVarMatrix);
        disc_val(3) = discriminant_func(X,Means{3},CoVarMatrix);
        disc_val(4) = discriminant_func(X,Means{4},CoVarMatrix);
