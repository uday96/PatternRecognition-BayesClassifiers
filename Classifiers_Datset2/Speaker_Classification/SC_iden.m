...
flag_FAR = 0;
while(1)
    targetmatrix = zeros(2,100);
    outputmatrix = zeros(2,100);
    pointer = 1;
    confusion_matrix = zeros(2);
    total_hits_test = 100;
    for i = 1:10
        for j = 1:10
            confusion_matrix = confusion_matrix + assign_class(Datasets_test(i),Means,Covars,PriorProbs,i,j);
        end
    end
    ...
    choose_THR(index,:) = [accuracy_test THR];
    ...
    FRR = confusion_matrix(1,2)/(confusion_matrix(1,1)+confusion_matrix(1,2));
    FAR = confusion_matrix(2,1)/(confusion_matrix(2,1)+confusion_matrix(2,2));
    fprintf('Accu: %f  THR: %0.f  FRR: %f   FAR: %f\n',accuracy_test,THR,FRR, FAR);
    disp(confusion_matrix);
    FAR_mat = [THR FAR accuracy_test;FAR_mat];
    FRR_mat = [THR FRR accuracy_test;FRR_mat];
    if(FRR >= 1 )
        flag_FRR = 1;
    end
    if(FAR >= 1 )
...
figure();
h = plot(choose_THR(:,2),choose_THR(:,1));
hold on
scatter(choose_THR(:,2),choose_THR(:,1),150,'filled', ...
...
[~,thr] = sort(choose_THR);
Sorted = choose_THR(thr,:);
l_k = length(Sorted(:,1))/2;
thr = Sorted(l_k,2);
accu_val = Sorted(l_k,1);
fprintf('Threshold: %f\n',thr);
% ConfusionMatrices(thr/50)
% fprintf('Classification Accuracy : %f\n',accu_val);
% 
% op = opMatrices(thr/50);
% tar = tarMatrices(thr/50);
% 
% figure();
% plotconfusion(tar,op);


function no_of_assignments = assign_class(datasets,Means,Covars,PriorProbs,Actualclasslabel,Givenclasslabel)
    global outputmatrix;
    global targetmatrix;
    global pointer;
    global THR;
%l_test = length(datasets(:,1));
    ...
    if(Actualclasslabel == Givenclasslabel)
        actualclass(1) = 1;
    else
        actualclass(2) = 1;
    end
    for i = 1:10
        ...
        %disp(disc_val);
        diff = getDiff(disc_val,Givenclasslabel);
        identified = 0;
        if(diff>THR && diff<-THR)
            identified = 1;
        end
        if(Actualclasslabel == Givenclasslabel && identified == 1)
            no_of_assignments(1,1) = no_of_assignments(1,1) + 1;
        end
        if(Actualclasslabel == Givenclasslabel && identified == 0)
            no_of_assignments(1,2) = no_of_assignments(1,2) + 1;
        end
        if(~(Actualclasslabel == Givenclasslabel) && identified == 1)
            no_of_assignments(2,1) = no_of_assignments(2,1) + 1;
        end
        if(~(Actualclasslabel == Givenclasslabel) && identified == 0)
            no_of_assignments(2,2) = no_of_assignments(2,2) + 1;
        end
...
function diff = getDiff(disc_val,class)
    mat = zeros(1,9);
    ptr = 1;
    val = disc_val(class);
    %disp(disc_val);
    ...
    %fprintf('val: %f  , mean: %f    diff: ',val,mean(mat));
    diff = (val - mean(mat));
    %disp(diff);
    %diff = abs(diff);
    %disp(diff);
