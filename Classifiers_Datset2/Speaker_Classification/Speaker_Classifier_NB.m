...
total_hits_test = 0;
for i = 1:10
    confusion_matrix(i,:) = assign_class(Datasets_test(i),Means,Covars,PriorProbs);
...
function no_of_assignments = assign_class(datasets,Means,Covars,PriorProbs)
...
function cov_matrix = get_cov(matrix)
    var_matrix = var(matrix);
    %disp(var_matrix);
    [~,di] = size(var_matrix);
    cov_matrix = zeros(di);
    for i = 1:di
        cov_matrix(i,i) = var_matrix(i);
    end
%     if(det(cov_matrix)== 0)
